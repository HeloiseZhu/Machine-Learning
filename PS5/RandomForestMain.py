...
def random_forest(train_x, train_y, test_x, test_y, T):
    m1 = train_x.shape[0]
    m2 = test_x.shape[0]
    prob = np.zeros(m2)
    for i in range(T):
        dt = DecisionTreeClassifier(min_samples_split=20, max_features='log2', random_state=1)#max_depth=20, 
        sample_index = sampling(m1)
        dt.fit(train_x[sample_index], train_y[sample_index].astype('int'))
...
def cross_validation(train_x, train_y, test_x, test_y, max_learner_num):
    ...
    for i in range(1, max_learner_num + 1):
        ...
        for train_index, test_index in kf.split(train_x):
            pred = random_forest(train_x[train_index], train_y[train_index], train_x[test_index], train_y[test_index], i)
...
if __name__ == "__main__":
    ...
    # 训练、测试
    pred = random_forest(train_x, train_y, test_x, test_y, learner_num)
